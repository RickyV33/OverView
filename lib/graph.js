    ...
    return new Promise((resolve, reject) => {
      ...
      let getData = () => {
        if (!timeout) {
          ...
          ++counter;
          let promises = [
            projectquire.getProjectName(this.projectId, this.url),
            projectquire.getProjectItems(this.projectId, this.url),
            projectquire.getProjectRelationships(this.projectId, this.url),
            projectquire.getProjectItemTypes(this.url)
          ];
          ...
          return Promise.all(promises).then(items => {
            ...
            // Add the image urls to each node depending on its type
            this.nodes.forEach(item => {
              let foundType = items[3].find(itemType => {
                return itemType.id === item.type ? itemType.image : null;
              });

              item.image = foundType.image;
            });
