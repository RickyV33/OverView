      ...
      let getData = () => {
        if (!timeout) {
          ...
          let promises = [
            ...
            projects.getProjectItemTypeImages(this.url),
            projects.getRelationshipTypeNames(this.url)
          ];
          ...
          return Promise.all(promises).then(items => {
            ...
            this.edges = items[2];
            let images = items[3];
            let edgeNames = items[4];

            // Maps the item type images to the node with a matching type
            this.nodes.forEach(node => {
              images.forEach(image => {
                for (let type in image) {
                  if (node.type === parseInt(type)) {
                    node.image = image[type];
                  }
                }
              });
            });

            // Maps the edge names to the node with a matching type
            this.edges.forEach(edge => {
              edgeNames.forEach(eName => {
                for (let type in eName) {
                  if (edge.type === parseInt(type)) {
                    edge.name = eName[type];
                  }
                }
              });
            });
