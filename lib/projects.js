 ...
 * promise is resolved, each iteration through the list of items pushes the ID, name, image, description and item type
 * into a temporary * node container, which ultimately becomes the value of the `nodes` data member. Else the rejected
 * promise issues a * message String used for error logging.
 *
 * @param {Number} projectId
 * @param {String} url Should be in the following format: http://username:password@teamname.jamacloud.com/rest/latest/
...
function getProjectItems (projectId, url) {
  ...
  return pagination(newUrl).then(items => {
    ...
    items.forEach(item => {
      let node = {
        'id': item.id,
        'documentKey': item.fields.documentKey,
      ...
      // Remove any and all html tags from the description
      node.description = sanitizeHtml(item.fields.description, {
        allowedTags: []
      });

      // Filter out the non-set item types
      if (!item.childItemType) {
...
/**
 * Retrieve all the item relationships types. Relationships types contain a name are then copied to each edge so that they may be
 * displayed in the graph on hover.
 *
 * @param url
 * @returns {Request}
 */
function getRelationshipTypeNames (url) {
  let newUrl = url + 'relationshiptypes';
  return pagination(newUrl).then(items => {
    let tempItemTypes = [];
    items.forEach(item => {
      let itemType = {};
      itemType[item.id] = item.name;
      tempItemTypes.push(itemType);
    });
    return tempItemTypes;
  }, error => Promise.reject(error));
...
module.exports = {
  ...
  getRelationshipTypeNames: getRelationshipTypeNames,
