...
/**
 * Grabs all the itemtypes for referencing the image URL's
 * @param url
 * @returns {Request}
 */
...
/**
 * Merges the item with the item types so we can access the images.
 * @param projectId
 * @param url
 * @returns {Request}
 */
function getProjectItemsWithImages (projectId, url) {
  let typeArray = getProjectItemTypes(url);
  let projects = getProjectItems(projectId, url);
  projects.forEach (item => {
    item.image = typeArray.find(itemType => {
      return itemType.id === item.type ? itemType.image : null;
    })
  });
  return projects;
}
...
module.exports = {
  ...
  getProjectItemsWithImages: getProjectItemsWithImages,
