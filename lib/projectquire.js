 ...
 * complete the request URL, hierarchy is called and returns the name of the project from the given arguments. If the
...
function getProjectName (projectId, url) {
  ...
  return pagination(newUrl).then(item => {
    // hierarchy stores payload in an array
    return item[0].fields.name;
  }, error => Promise.reject(error));
}

/**
 * Retrieve all items from some project given its project ID and the associated url. Prior to calling the hierarchy
 ...
 * hierarchy function, `url` is appended with the appropriate suffix to complete the request URL for all project item
