...
/**
 * Retrieves all item types from the team instance of Jama, then associates a URL to the item type image to that item
 * type id. It resolves a promise if it successfully retrieved all the images, otherwise it rejects an error.
 *
 * @param url url Should be in the following format: http://username:password@teamname.jamacloud.com/rest/latest/
 * @returns {Request} A resolved promise with the item type images of the project or a rejected promise with an error
 */
function getItemImages (url) {
  let newUrl = url + 'itemtypes';
  return pagination(newUrl).then(items => {
    let imageNodeContainer = [];
    items.forEach(item => {
      let node = {};
      node[item.id] = item.image;
      imageNodeContainer.push(node);
    });
    return imageNodeContainer;
  }, error => Promise.reject(error));
}
