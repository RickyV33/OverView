...
function getProjectName (projectId, url) {
  ...
  function gatherData() {
    ...
    let name = pagination.pagination(newUrl, 0, Number.MAX_SAFE_INTEGER);
    projectName = name.then((item) => {
      ...
      return gatherData(++counter);
    });
  }
  // for (let i = 0; success && i < 5;) {
  //   let name = pagination.pagination(newUrl, 0, Number.MAX_SAFE_INTEGER);
  //   projectName = name.then((item) => {
  //     // pagination stores payload in an array
  //     return item[0].fields.name;
...
function getProjectItems (projectId, url) {
  ...
  let nodeItems = pagination.pagination(newUrl, 0, Number.MAX_SAFE_INTEGER);
...
function getProjectRelationships (projectId, url) {
  ...
  let relationshipItems = pagination.pagination(newUrl, 0, Number.MAX_SAFE_INTEGER);
