...
describe('Pagination Module', () => {
  ...
  describe('Pagination Function', () => {
    ...
    it('should reject if the username is invalid', () => {
      ...
      expect(pagination('http://notdummy:password@sevensource.jamacloud.com/rest/latest/projects',
        startAt, maxPossible)).to.eventually.be.rejected();
    });
    //  Test #2: password is invalid
    //  returned promise from pagination should be rejected
    it('should reject if the password is invalid', () => {
      pagination = proxyquire('../../lib/pagination', { 'request': rejectedRequestStub });
      expect(pagination('http://dummy:invalidPassword@sevensource.jamacloud.com/rest/latest/projects',
        startAt, maxPossible)).to.eventually.be.fulfilled();
    });
    //  Test #3: URL contains a typo (is invalid)
    //  returned promise from pagination should be rejected
    it('should reject if the URL is invalid', () => {
      pagination = proxyquire('../../lib/pagination', { 'request': rejectedRequestStub });
      expect(pagination('http://dummy:password@sevensourcejamacloud.com/rest/latest/projects',
        startAt, maxPossible)).to.eventually.be.rejected();
    });
    ...
    it('should reject if the startAt value is invalid (less than zero)', () => {
      pagination = proxyquire('../../lib/pagination', { 'request': rejectedRequestStub });
      expect(pagination('http://dummy:password@sevensourcejamacloud.com/rest/latest/projects',
        -1, maxPossible)).to.eventually.be.rejected();
    });
    ...
    it('should return an empty page when URL is valid, startAt is valid, and maxResultsAllowed is 0 ', () => {
      ...
      expect(pagination('http://dummy:password@sevensource.jamacloud.com/rest/latest/projects',
        startAt, maxPossible)).to.eventually.be.fulfilled().and.to.equal(arrayOfPageData(pages));
    });
    ...
    it('should return page(s) of less than 20 items when URL is valid, startAt is valid, maxResultsAllowed ' +
      'is less than 20, and there are results to retrieve', () => {
    ...
    //  returned promise from pagination should be valid and equal to pages' data
    it('should return page(s) when URL is valid, startAt is valid, and maxResultsAllowed is greater ' +
      'than maximum allowed (20), and there are 20 results to retrieve ', () => {
    ...
    //  returned promise from pagination should be valid and equal to pages' data
    it('should return a single page of results when URL is valid, startAt is valid, maxResultsAllowed <= 20, ' +
      'and there are < 20 results to retrieve ', () => {
